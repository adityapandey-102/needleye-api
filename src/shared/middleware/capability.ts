 ...
 * all. When the role's access is scoped to "assigned" records, the route
 * handler is responsible for filtering/verifying ownership (designer_id /
 * master_tailor_id / order's designer for payments) -- this middleware only
 * gates the all-or-nothing role check and records the scope on `req` so
 * handlers don't have to re-derive it.
 */
